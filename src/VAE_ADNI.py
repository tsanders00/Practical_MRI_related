...
def evaluate_model(vae, models, x_test):
    ...
    for i in range(num_samples):
        plt.figure(figsize=(120, 16))  # Adjust the overall figure size
...
if __name__ == '__main__':
    ...
    with tf.device('CPU'):
        print("Loading data")
        images = load_npy(directory='/home/tcsn39/adni/Datenneu/CN_axial_npy/*.npy')
        print(f'Number of images in dataset: {len(images)}')
        images = filter_arrays_for_small_cube(arrays=images)
        print(f'Number of images left in dataset after removing small cubes: {len(images)}')
        images = filter_arrays_for_black_cube(arrays=images)
        print(f'Number of images left in dataset after removing all black cubes: {len(images)}')
    ...
    fname = "/home/tcsn39/vae/plots/loss"
    plot_history(history=history.history, fname=fname)

    evaluate_model(vae=vae, models=models, x_test=x_test)

    generate_and_test_images_with_cnn(vae=vae, x_test=x_test, number_of_pictures=50,
